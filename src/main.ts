...
// add svgs as resolvable shapes to party.js
shapes.forEach(shape => {
    party.resolvableShapes[shape.name] = shape.svg;
})
    ...
    if (!firstPick) {
        ...
        if (firstShapeName !== secondShapeName) {
            ...
            if (matches === 8) {
                ...
                reward.classList.toggle("hidden");
                audioEl.play();
                // it seems, that dividing window inner sizes by more than 4 doesn't make a difference
                const x = window.innerWidth / 8;
                const y = window.innerHeight / 8;
                party.confetti(new party.Rect(x, y, 50, 50), {
                    count: party.variation.range(0, 100),
                    size: party.variation.range(0.1, 0.2),
                    shapes: firstShapeName,
                })
                setTimeout(() => {
                    reward.classList.toggle("hidden");
                }, 2000);
